...
import torch
#  This module contains a pure-Python description of the Google Cloud TPU job API, as generated by the google.api.generator.proto2.py plugin for the Google protobuf compiler.
import fedscale.core.channels.job_api_pb2 as job_api_pb2

#  common functions used across the FedScale system.
from fedscale.core import commons
#  is used to stores the context for a particular channel.
...
#         这里应该是当data traffic足够大时，需要自己去configure调用的
    ...
    def init_model(self):
        ...
        """
#       This function is responsible for initializing the model for training. It first checks to see if the engine is set to PyTorch, and if it is,
# it initializes the model and returns it. Otherwise, it raises an error.
...
#         https://github.com/SymbioticLab/FedScale/search?q=rl
        if self.task == "rl": #reinforcement learning
...
# collate used to process the data, 
...
#             t here is used to send a message to the aggregator that the training process for the client has completed. 
#             This is used so that the aggregator can keep track of the training progress for each client.
...
#  This is done so that the executor has the latest version of the model 
# that is being trained
    def update_model_handler(self, model):
        ...
        """
        self.model = model
#        keep track of the number of training rounds that have been completed.
        self.round += 1

        
        # Dump latest model to disk, The 'wb' means write binary. 
        # This is used so that the model can be pickled and written to disk.
    ...
    def load_global_model(self):
        ...
        """
        with open(self.temp_model_path, 'rb') as model_in:  #read the binary file
...
#         跳到from fedscale.core.execution.client import Client
    ...
    def training_handler(self, clientId, conf, model=None):
        ...
        """
        # load last global model
# it will either use the last global model or the model that was passed in.
# This is used so that the training process can continue from the last global model 
# or from a different model if one is provided.
...
# If the task is rl, then the code will train the model using the RLClient class. 
# Otherwise, the code will train the model using the get_client_trainer class.
...
#     This is used so that the aggregator can keep track of the training progress and their status. for each client.
    def client_register(self):
        ...
        while time.time() - start_time < 180:
            try:
#                 used so  that the aggregator knows any executor is ready to start training
...
#         almost the same like above function.
        """Ping the aggregator for new task
        """
#         to check if there are any new tasks. this is for after one new round to check any client.
#         or say send a message to the aggregator that the executor is still alive and ready to train
...
#          monitor events from the aggregator,the executor can respond to messages from the aggregator.
...
#         The function loops through the event queue and dispatches the appropriate event handler for each event.
        while self.received_stop_request == False:
            if len(self.event_queue) > 0:
                request = self.event_queue.popleft()
                current_event = request.event

# The function loops through the event queue and dispatches the worker events. 
# 1.If the event is a train event, then the executor will train the model and 
# upload the results to the aggregator. 
# 2.If the event is a test event, then the executor will test the model. 
# 3.If the event is an update model event, then the executor will update the local copy of the model.
# 4.If the event is a shut down event, then the executor will stop training.         
